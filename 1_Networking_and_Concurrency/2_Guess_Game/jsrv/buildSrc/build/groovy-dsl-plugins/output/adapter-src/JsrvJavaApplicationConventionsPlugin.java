...
/**
 * Precompiled jsrv.java-application-conventions script plugin.
 **/
public class JsrvJavaApplicationConventionsPlugin implements org.gradle.api.Plugin<org.gradle.api.internal.project.ProjectInternal> {
    ...
    public void apply(org.gradle.api.internal.project.ProjectInternal target) {
        assertSupportedByCurrentGradleVersion();
                target.getPluginManager().apply("jsrv.java-common-conventions");
        target.getPluginManager().apply("application");

        try {
            Class<? extends BasicScript> precompiledScriptClass = Class.forName("precompiled_JsrvJavaApplicationConventions").asSubclass(BasicScript.class);
